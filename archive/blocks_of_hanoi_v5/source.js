...
window.onload = function init() {
   ...
   canvas.addEventListener("wheel", function(event){
     ...
     if ((zoomLevel + delta >= -5) && (zoomLevel + delta <= 5)) {
         zoomLevel += delta;
      }
  });
...
var render = function () {
   ...
   // SHARED PROPERTIES
   eye = vec3(12 + zoomLevel, 4 + zoomLevel, 12 + zoomLevel);
   mvMatrix = lookAt(eye, at, up);
   modelView = mvMatrix;
   if (trackballMove) {
      axis = normalize(axis);
      rotationMatrix = mult(rotationMatrix, rotate(angle, axis));
      //rotationMatrix = mult(rotationMatrix, translation);
   }
